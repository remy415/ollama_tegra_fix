...
if [ -d "${CUDA_LIB_DIR}" ]; then
    ...
    if [ "${CUDA_TEGRA}" == "TEGRA" ]; then
        CMAKE_DEFS="-DCMAKE_POSITION_INDEPENDENT_CODE=on -DCMAKE_CUDA_COMPILER=/usr/local/cuda/bin/nvcc -DLLAMA_NATIVE=off -DLLAMA_AVX=off -DLLAMA_AVX2=off -DLLAMA_AVX512=off -DLLAMA_FMA=off -DCMAKE_CUDA_STANDARD=17 -DLLAMA_CUBLAS=on -DLLAMA_CUDA_F16=1 -DCMAKE_CUDA_ARCHITECTURES=${CMAKE_CUDA_ARCHITECTURES}"
    else
